...
namespace binon {
	inline namespace literals {
		...
		constexpr auto operator ""_byte(unsigned long long i) {
				static_assert(i < 0xffU, "_byte literal out of range");
				return ToByte(i);
			}
		...
		constexpr auto operator ""_f32(long double x) {
				static_assert(
					x >= std::numeric_limits<TFloat32>::lowest &&
					x <= std::numeric_limits<TFloat32>::max,
					"_f32 literal out of range"
					);
				return static_cast<TFloat32>(x);
			}
		constexpr auto operator ""_f64(long double x) {
				static_assert(
					x >= std::numeric_limits<TFloat64>::lowest &&
					x <= std::numeric_limits<TFloat64>::max,
					"_f64 literal out of range"
					);
				return static_cast<TFloat64>(x);
			}
