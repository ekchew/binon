...
namespace binon {
	...
	*/
	...
	/*
	The BinONObj struct is central to the BinON C++ implementation.
	As a BinONVariant, it can be any of the object types. BinONObjs are managed
	by many of the APIs in this library, and the struct itself comes with some
	convenience methods for encoding/decoding BinON and so forth.
	*/
	struct BinONObj: BinONVariant
	{
		/*
		The Decode() class method can decode an arbitrary BinON object from an
		input binary stream.
		...
		Args:
			...
			requireIO: throw exception if a read operation fails?
				...
				If you are doing several encoding/decoding operations in a row,
				however, you might consider declaring a RequireIO object
				(see ioutil.hpp). This will set the flags while it exists and
				restore them when it destructs. Then you can call
		...
		*/
		...
		/*
		FromTypeCode() returns a new object of the type specified by type code
		(see codebyte.hpp for a list of possible codes). This object type will
		...
		*/
		...
		Perhaps the easiest way is to call MakeBinONObj() (see typeconv.hpp):
		...
		*/
		...
		/*
		As you would expect, the encode() method encodes a BinON representation
		of the current object to an output binary stream.

		Note that each specific BinON object class (e.g. StrObj) has its own
		specialized encode() method. This encode() simply calls the appropriate
		one.
		*/
		...
		object data. You can call this if the object type is clear by context.
		For example, the ListObj class encodes the length of the list with
		UIntObj's encodeData(). It doesn't need to encode the type since the
		size of a list will always be an unsigned integer.
		*/
		...
		instance method--not a class method like Decode(). That means you need
		to have allocated a BinONObj of the correct type before you can call
		decodeData() on it.
		*/
...
namespace std {
	...
	/*
	Any scalar BinONObj can be hashed or equivalence-compared, meaning it can
	be used as a key in any of the dictionary container classes. At this time,
	container types themselves cannot be hashed, though this could change?

	Note that individual BinON object classes like StrObj cannot be hashed
	directly by std::hash, but they do provide hash() methods that should return
	a suitable hash code (other than for the aforementioned container types
	whose hash() methods will throw an exception instead).
	*/
