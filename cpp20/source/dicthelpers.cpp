...
namespace binon {
	...
	auto MakeSKDict(
		...
		auto& map = obj.value();
		for(auto& pair: pairs) {
			map[std::move(pair.first).asTypeCodeObj(keyCode)] = std::move(pair.second);
		}
		return obj;
	}
	auto MakeSDict(
		...
		auto& map = obj.value();
		for(auto& pair: pairs) {
			map[std::move(pair.first).asTypeCodeObj(keyCode)] =
				std::move(pair.second).asTypeCodeObj(valCode);
		}
		return obj;
	}
}
