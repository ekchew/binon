...
namespace binon {
	...
	//
	template<std::convertible_to<std::size_t>... Vs> constexpr
		auto HashCombine(std::size_t v, Vs... vs) noexcept {
			...
			//	called "golden ratio", this has been extended to 64 bits for
			//	size_t values > 32 bits.
			auto combine2 = [](size_t a, size_t b) constexpr noexcept
				-> size_t
			{
				size_t magic = std::numeric_limits<size_t>::digits > 32U ?
					0x9e3779b97f4a7c15U : 0x9e3779b9U;
	...
	class CommutativeHash {
		std::size_t _hash = 1927868237U;

	 public:

		//	The extend() method combines a new hash value with the one that is
		//	currently stored.
		constexpr void extend(std::size_t h)
			{ _hash ^= (h ^ (h << 16) ^ 89869747UL) * 3644798167UL; }
		...
		template<Hashable T> void operator() (const T& v)
			{ this->extend(std::hash<T>{}(v)); }
		...
		//	CommutativeHash also implements a conversion operator to size_t
		//	that calls get implicitly.
		auto get() const -> std::size_t {
				auto h = _hash;
				h = h * 69069U + 907133923UL;
				//if(h == -1) { h = 590923713UL; }
				return h;
			}
