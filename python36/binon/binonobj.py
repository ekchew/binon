...
class BinONObj:
    ...
    def BaseObj(cls, value):
        """
        Before you can encode a given value in BinON, you need to identify
        which codec class can handle values of that type. The codec classes are
        ...
        optimal. For example, BinONObj.BaseObj(100) will return an IntObj, but
        a UInt (a subclass of IntObj) would give you a shorter encoding. If you
        want a UInt instead, you can either call OptimalObj() or simply wrap
        ...
        Args:
            value (object): a value you want to encode
                ...
                of BinONObj. (In the latter case, value will be returned
                as-is.)
    ...
    def OptimalObj(cls, value, inList=False):
        """
        OptimalObj() tries to find the best possible codec class for your value
        by examining it more closely than BaseObj() does. For example, it would
        realize that 100 would be better encoded as a UInt than a more general
        signed IntObj.

        This optimization does incur some extra processing overhead, however,
        particularly when dealing with container types (e.g. determining
        whether an SList could be substituted for a ListObj).
        ...
        Args:
            ...
            inList (bool, optional): value is a list element?
                ...
                with False by default. It is pertinent to the case in which
                your value is the default value for the codec in question. For
                ...
                so BinONObj.OptimalObj(0.0, inList=True) should return a
                Float32 instead.
    ...
    def Decode(cls, inF, asObj=False):
        ...
        Args:
            ...
            asObj (bool, optional): leave return value in object form?
                ...
                leaves the wrapper intact. This could save some time if you
                need to re-encode sometime later?
    ...
    def encodeData(self, outF):
        ...
        call the correct DecodeData method to get your object back. For
        example, if you called intobj.UInt(42).encodeData(outF), you need to
        call intobj.UInt.DecodeData(inF) to get your 42 back.
